...
3. **Executor (Execution Agent)**: A lightweight/local LLM (DeepSeek-V4-Flash - default, gpt-5.4-nano - alternative) that performs incident remediation actions based on the Judge's decision, wrapped with a Human-in-the-Loop CLI approval.
...
- `EXECUTOR_MODEL`: Model for the Executor Agent (default: DeepSeek-V4-Flash)
